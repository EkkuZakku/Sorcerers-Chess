...
public class GameLogic : MonoBehaviour
{
    ...
    private void PlacePieces()
    {
        for (int i = 0; i <= 7; i++)
        {
            for (int j = 0; j <= 7; j++)
            {
                ChessTypes.XY location = new ChessTypes.XY(i, j);
                if (game_board.Get_Piece_At_Location(location).PieceType != ChessTypes.Pieces.None)
                {
                    CreatePiece(location, game_board.Get_Piece_At_Location(location));
    ...
    public static void CreatePiece(ChessTypes.XY location, ChessTypes.Piece piece)
    {
        GameObject newpiece = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
        newpiece.name = ChessTypes.BoardXInverse[location.x] + ChessTypes.BoardYInverse[location.y] + ChessTypes.PieceTypeInverse[piece.PieceType];
        if (piece.Color == ChessTypes.Pieces.Black)
            newpiece.GetComponent<Renderer>().material = Resources.Load("Standard Assets/Environment/TerrainAssets/SurfaceTextures/MudRockyAlbedoSpecular", typeof(Material)) as Material;
        newpiece.AddComponent<PieceHandler>();
        PieceHandler newpiecehandler = newpiece.GetComponent<PieceHandler>();
        newpiecehandler.location = location;
        newpiece.transform.position = new Vector3((float)newpiecehandler.location.x, 0.5f, (float)newpiecehandler.location.y);
